...
#======================================================================
    ...
    def create_table(self):
        ...
        # Migración para la columna 'marca'
        try:
        ...
        # 2. Crea la tabla usuarios (Corregida: sin UNIQUE en 'rol')
        self.cursor.execute('''
...
#======================================================================
    ...
    # Si el medicamento ya existe por nombre (ignorando mayúsculas/minúsculas), 
    # suma la cantidad al stock existente.
    
    def registrar_medicamento(self, nombre, cantidad, fecha_vencimiento, marca):
        # 1. NORMALIZAR EL NOMBRE: Convertir todo a mayúsculas y quitar espacios extra
        nombre_normalizado = nombre.strip().upper()
        
        cursor = self.conn.cursor()

        try:
            # 2. Buscar usando el nombre ya normalizado (en mayúsculas)
            # Se usa UPPER(nombre) por si hay registros antiguos en la BD que estén en minúsculas
            cursor.execute("SELECT id, cantidad FROM inventario WHERE UPPER(nombre) = ?", (nombre_normalizado,))
            ...
            if resultado:
                ...
                self.conn.commit()
                print(f"Stock actualizado para {nombre_normalizado}. Nuevo total: {nuevo_stock}")
            else:
                # 3. Guardar el nuevo registro usando el nombre normalizado
                cursor.execute(
                    "INSERT INTO inventario (nombre, cantidad, fecha_vencimiento, marca) VALUES (?, ?, ?, ?)",
                    (nombre_normalizado, cantidad, fecha_vencimiento, marca)
                )
                self.conn.commit()
                print(f"Nuevo medicamento {nombre_normalizado} registrado con éxito.")
...
#======================================================================
 ...
 #======================================================================    
    ...
    # agregar_usuario: Permite registrar un nuevo usuario en la base de datos
    def agregar_usuario(self, rol, usuario, password):
        # Genera el hash de la contraseña de la misma forma que los usuarios iniciales
        password_hash = hashlib.sha256(f"{usuario}:{password}".encode()).hexdigest()
        
        try:
            # Intenta insertar el nuevo usuario
            self.cursor.execute(
                "INSERT INTO usuarios (rol, usuario, password_hash) VALUES (?, ?, ?)",
                (rol, usuario, password_hash)
            )
            self.conn.commit()
            return True, "Usuario creado exitosamente."
        except sqlite3.IntegrityError:
            # sqlite3.IntegrityError se dispara si el 'usuario' ya existe por la restricción UNIQUE
